...
{
    [Header("References")]
    [SerializeField] private Transform player;   // Player referansı
    ...
    [SerializeField] private float spawnAheadDistance = 25f;   // Player’ın önünde ne kadar ileride spawn
    [SerializeField] private float minDistanceBetweenSpawns = 3f;
    [SerializeField] private float maxDistanceBetweenSpawns = 6f;

    [Header("Y Pozisyonları")]
    [SerializeField] private float groundY = 2f;      // Engel için zemin yüksekliği
    ...
    private void Start()
    {
        // Player atanmadıysa tag üzerinden bul
        if (player == null)
        {
    ...
    private void Update()
    {
        ...
        // Player’ın ilerisinde, şu kadar mesafeye kadar her frame spawn’ı doldur
        ...
        while (nextSpawnX < targetX)
        {
            ...
            // Oyun hızlandıkça aralıkları biraz kısalt (opsiyonel, ama güzel his verir)
            float multiplier = 1f;
